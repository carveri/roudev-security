...
const TablaEmpleados = ({res, bears, userId}) => {
  ...
  console.log('reultado empleados:', res);
    ...
    const router = useRouter()
      ...
      const handleClickLapiz2 =()=>{
        setActivarEliminar(!activarEliminar)
      }

      const handleClickEliminar2 =()=>{
        setActivarEliminar(!activarEliminar)
      }

      const handleClickBuscar2 =()=>{
          ...
          <tbody>
            {userId?.map((el, items)=>{
                  ...
                  <td className='w-full  grid place-content-center  h-11'>
                    {<img src={`${!el?.avatar ? 'https://roudev-s3-assets.s3.us-east-1.amazonaws.com/AssetsRoudev/Icons/nextjs.png' : el?.avatar}`} width={17} height={17} alt='d' />}
                  </td>
                  <td className='text-center'>
                    {el?.primerNombre}
                  </td>
                  ...
                  <td onClick={()=>router.push('/dashboard/perfil/perfilPersonal')} className=' text-center text-[11px] text-blue-500 cursor-pointer hover:border-b hover:border-blue-500 font-extrabold'>
                    ...
                    <td onClick={handleClickLapiz2} className=' py-[11px] h-full place-items-center cursor-pointer hover:border-b hover:border-blue-500'>
                      <img  src='https://roudev-s3-assets.s3.us-east-1.amazonaws.com/AssetsRoudev/Icons/lapiz.png' width={13} height={13} alt='delete' />
                    </td>
                    <td onClick={handleClickEliminar2} className='  py-[11px] h-full place-items-center cursor-pointer hover:border-b hover:border-blue-500'>
                      <img  src='https://roudev-s3-assets.s3.us-east-1.amazonaws.com/AssetsRoudev/Icons/eliminar.png' width={13} height={13} alt='delete' />
                    </td>
                    <td onClick={handleClickBuscar2} className='grid py-[11px] h-full justify-center cursor-pointer hover:border-b hover:border-blue-500'>
