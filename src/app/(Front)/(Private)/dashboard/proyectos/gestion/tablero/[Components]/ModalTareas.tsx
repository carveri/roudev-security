  ...
  // ESTADOS DESDE ZUSTAND
      ...
      const handleClickCheckBox =()=>{
        setActivarCheck(!activarCheck)
              ...
              // TRAER LA TAREA SELECCIONADA POR SU ID
              const getTareaSeleccionado = async()=>{
                const ruta = 'tarea'
                const url = tareaSeleccionadaId
              ...
              useEffect(()=>{
                  ...
                  // console.log('sd', tareaSelecc[0]);
                  // console.log('usuario seleccici', userId);
                  // console.log('idfinal', idResponsableTarea);
                  // console.log('tipoidfinal', typeof(idResponsableTarea));
                  
                  
            // boton para abrir aceptar cambios
  const handleClickEnviarPeticion =(e, nombreResponsable)=>{
    e.preventDefault()
    ...
    setAbrirModalTareas(false)
      ...
      return (
        <div className="z-10">
            <form  className={`${aceptarColaborador ? ('opacity-5'): ('opacity-100')} w-[23%] h-[82%] py-4 px-10  rounded-md mt-28  fixed top-14 ml-[1350px] grid  text-tamañoLetraChica`}>
                  <header className='w-full h-[10%] capitalize grid place-content-center  text-gray-600 font-semibold text-tamañoLetra '>
                  ...
                  <main className='h-[50%]  -mt-8 '>
                    <Link href={'/dashboard/perfil/perfilLaboral'} className='grid'>
                      ...
                      <span className="py-4 flex items-center pl-1 text-gray-700 hover:text-blue-500 hover:bg-gray-50">{tareas[0]?.user?.primerNombre} {tareas[0]?.user?.apellidoPaterno}</span>
                    </Link>
                    <Link href={'/dashboard/perfil/perfilLaboral'} className='grid'>
                      <label className='h-1 text-gray-600 pb-5' htmlFor="">Responsable de la Tarea</label>
                      <span className="py-4 flex items-center pl-1 text-gray-700 hover:text-blue-500 hover:bg-gray-50">{userId?.primerNombre} {userId?.apellidoPaterno}</span>
                    </Link>
                    <div className='grid '>
                      <label className='h-1 text-gray-600 pb-5' htmlFor="">Equipo del Responsable</label>
                      <div className="py-4 flex items-center pl-1">{tareas[0]?.equipo?.nombreEquipo}</div>
                    </div>
                    <div className='grid '>
                      <label className='h-1 text-gray-600 pb-5' htmlFor="">Urgencia de la Tarea</label>
                      <div className={`${tareas[0]?.urgenciaTarea ==='Alta' ? 'text-red-600' : tareas[0]?.urgenciaTarea ==='Media' ?'text-yellow-500' : tareas[0]?.urgenciaTarea ==='Baja' ?'text-green-400' : ''} py-4 flex items-center pl-1  font-bold`}>{tareas[0]?.urgenciaTarea}</div>
                    </div>
                    <div className='grid '>
                      <label className='h-1 text-gray-600 pb-5' htmlFor="">Fechas requeridas para completar la Tarea</label>
                      <div className="py-4 flex items-center pl-1">{tareas[0]?.fechaInicio} - {tareas[0]?.fechaTermino}</div> 
                    </div>
                    <div className='grid '>
                      <label className='h-1 text-gray-600 pb-5' htmlFor="">Nivel de Avance de la Tarea</label>
                      <div className="py-4 flex items-center pl-1 font-semibold text-blue-500">{tareas[0]?.nivelDeAvance}</div>
                    </div>
                    <div className='grid '>
                      <label className='h-1 text-gray-600 pb-5' htmlFor="">Descripción de la Tarea</label>
                      <div className="py-4 flex items-center pl-1">{tareas[0]?.descripcionDeTarea}</div>
                    </div>
                  ...
                  <div className='h-[48%] w-full flex justify-end items-end gap-x-2'>
                    
                    <button onClick={(e)=>handleClickEnviarPeticion(e, userId?.primerNombre)} disabled={!activarCheck}  className={`h-[100%] w-[35%]  rounded cursor-pointer ${activarCheck ? ('opacity-100'): ('opacity-10 cursor-text')} bg-blue-500 hover:bg-blue-600  hover:duration-300  text-white font-bold text-tamañoLetraChica`}>
                      Enviar Petición
                    </button>
